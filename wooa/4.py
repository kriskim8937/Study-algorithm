...
def solution(pobi, crong):
    ...
    if crong[0] + 1 != crong[1]:
        return -1
    if 1 < pobi[0] < 399 and 2 < pobi[1] < 400 and 1 < crong[0] < 400 and 2 < crong[1] < 400:
        pops = max(sum_digit(pobi[0]),sum_digit(pobi[1]),multi_digit(pobi[1]),multi_digit(pobi[0]))
        cros = max(sum_digit(crong[0]),sum_digit(crong[1]),multi_digit(crong[1]),multi_digit(crong[0]))
        if pops > cros:
            answer = 1
        elif cros > pops :
            answer = 2 
